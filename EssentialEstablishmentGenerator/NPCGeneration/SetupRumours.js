...
setup.createRumour = function (town) {
  ...
  function MajorQuest () {
    return [
      `awaken a sleeping NPC called ${State.setVar(`Quest`, createNPC(town))}`,
      `${[`recover`, `destroy`].seededrandom()} an artifact ${ItemInformation()}`,
      [`aid`, `slay`] + State.setVar(`Quest`, createNPC(town)),
      `slay a monster`,
      [`liberate`, `enslave`] + State.setVar(`Quest`, createNPC(town)),
