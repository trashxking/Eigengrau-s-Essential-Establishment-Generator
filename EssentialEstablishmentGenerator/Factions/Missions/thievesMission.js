...
export function thievesMission (town, mission) {
  mission.type = randomValue([`alchemy`, `assassination`, `blackmail`, `burglary`, `swindle`, `fence`, `fraud`, `gambling`, `kidnapping`, `narcotics`, `prostitution`, `protection`, `slavery`, `smuggling`, `street`])

  const details = missionDetails(mission)

  if (details) {
    mission.details = randomValue(details)
  }

  mission.difficultyText = missionDifficulty(mission)

  const openingLine = randomValue([
    `${randomValue([
      `I've got a job for you.`,
      `Got a job for you to do.`,
      `There's a job I need you to do.`,
      `There's something I need you to do.`
    ])} ${mission.difficultyText} ${randomValue([`This one's a `, `It's a `, `This is a `])}${mission.type} job.`,
    `${randomValue([
  ...
  const closingLine = randomValue([`fuck it up.`, `let me down.`, `disappoint me.`])
