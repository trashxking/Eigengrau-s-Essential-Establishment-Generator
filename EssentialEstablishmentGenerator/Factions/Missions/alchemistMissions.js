...
const missions = {
  disappearingPotions (town) {
    const npc = createNPC(town, { dndClass: `rogue`, background: `criminal` })
  ...
  testSubject () {
    const missionPotion = createAlchemy({ type: `potion` })
  ...
  bandits (town) {
    const npc = createNPC(town, { dndClass: [`rogue`, `fighter`].seededrandom(), background: `criminal` })
    const missionIngredient = createAlchemy({ type: `preserved herb` })
  ...
  bodyParts () {
    const missionBodyPart = createAlchemy({ type: `body part` })
  ...
  badIngredients (town) {
    const missionIngredient = createAlchemy({ type: `preserved herb` })
    const npc = createNPC(town, { hasClass: false, profession: `alchemist` })
