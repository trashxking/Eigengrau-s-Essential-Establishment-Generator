...
export function createBlacksmith (town, smithy) {
  ...
  return createNPC(town, {
    ...
    currentproject: randomValue([
      `a ${randomValue(smithy.weapons)} for a lord's son.`,
      `a ${randomValue(smithy.weapons)} which I'm hoping to sell on the market in a couple day's time.`,
      `a ${randomValue(smithy.weapons)} which I'm hoping to sell on the market in a couple day's time.`,
      ...
      `a set of horseshoes for ol' Bessie. Nah, I'm just messin' with ya. I'm working on a ${randomValue(smithy.weapons)}. Should be pretty fun once it's finished!`,
      `a ${randomValue(smithy.weapons)} for my son, he's growing up so fast- he'll be six next winter!`,
      `a ${randomValue(smithy.weapons)} for the captain of the guard, ${town.guard.captain.name}.`,
      `a ${randomValue(smithy.weapons)} for the captain of the guard, ${town.guard.captain.name}.`,
      `a commission by the town guard to make new ${randomValue(smithy.weapons)}s for all the guardsmen. It'll probably take a damn long time.`,
