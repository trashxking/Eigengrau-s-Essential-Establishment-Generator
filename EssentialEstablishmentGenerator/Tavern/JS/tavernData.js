...
setup.tavern = {
  ...
  get: {
    ...
    carousing (town, tavern) {
      const carousing = {
        ...
        debt (town) {
          const npc = createNPC(town, { background: `noble`, hasClass: false, isThrowaway: true })
          return `You have agreed to take on a noble called ${setup.profile(npc, npc.firstName)}'s debts.`
        },
        joust (town) {
          const npc = createNPC(town, { dndClass: `fighter`, background: `soldier`, gender: `man`, isThrowaway: true })
    ...
    draws (town, tavern) {
      ...
      const draws = [
        ...
        },
        {
          ...
          //   tavern.bard = createNPC(town, { dndClass: 'bard', gender: 'man' })
          //   return {
          //     tavern
          //   }
          // },
        ...
        },
        {
          ...
          //   tavern.patron = createNPC(town, { hasClass: false, gender: 'man' })
          //   return {
          //     tavern
          //   }
          // },
        ...
        //     tavern.roll.magic += 10
        //     var drunkWizard = createNPC(town, {dndClass: 'wizard' })
        //     return {
  ...
  patrons: {
    farmer (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `farmer`, gender: `man` })
      return `A poor ${setup.profile(npc, `farmer`)} is trying to pay his bar tab with a sack of potatoes and a barrel of pickles.`
    },
    seacaptain (town) {
      const npc = createNPC(town, { background: `sailor`, profession: `ship's captain`, gender: `man` })
    ...
    ranger (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `hunter`, dndClass: `ranger`, gender: `man` })
    ...
    troll (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `drunkard`, gender: `man` })
    ...
    wizard (town) {
      const npc = createNPC(town, { background: `sage`, profession: `wizard`, dndClass: `wizard`, gender: `man` })
      return `A stoned ${setup.profile(npc, `magician`)} is fumbling his way through one awful card trick after another.`
    },
    elves (town) {
      const npc = createNPC(town, { background: `noble`, profession: `noble`, race: `elf`, gender: `woman`, weapon: `an obsidian bow` })
    ...
    clown (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `clown` })
      return `A sad ${setup.profile(npc, `clown`)} is drinking a beer alone.`
    },
    guards (town) {
      const npc = createNPC(town, { background: `soldier`, profession: `guard`, dndClass: `fighter` })
    ...
    suitor (town) {
      const npc = createNPC(town, { background: `noble`, profession: `noble`, hasClass: false, gender: `woman` })
    ...
    ladies (town) {
      const npc = createNPC(town, { background: `scholar`, profession: `librarian`, gender: `woman` })
    ...
    witch (town) {
      const npc = createNPC(town, { background: `hermit`, profession: `witch`, dndClass: `wizard`, gender: `woman` })
      return `A surly old ${setup.profile(npc, `witch`)} is drinking sherry while stroking the fat grey cat sitting on her lap.`
    },
    marilith (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `barmaid`, gender: `woman` })
    ...
    mophand (town) {
      const npc = createNPC(town, { background: `commoner`, profession: `maid-servant`, hasClass: false })
    ...
    priest (town) {
      const npc = createNPC(town, { background: `sage`, profession: `priest`, dndClass: `cleric`, weight: `fat`, weightRoll: 200 })
